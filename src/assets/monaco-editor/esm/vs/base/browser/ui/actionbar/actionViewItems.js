...
export class BaseActionViewItem extends Disposable {
  ...
  render(container) {
    ...
    if (enableDragging) {
      ...
      if (isFirefox) {
        ...
        this._register(
          DOM.addDisposableListener(container, DOM.EventType.DRAG_START, (e) => {
            var _a;
            return (_a = e.dataTransfer) === null || _a === void 0
              ? void 0
              : _a.setData(DataTransfers.TEXT, this._action.label);
          })
        );
      }
    }
    ...
    this._register(
      DOM.addDisposableListener(element, DOM.EventType.MOUSE_DOWN, (e) => {
        if (!enableDragging) {
    ...
    if (platform.isMacintosh) {
      ...
      this._register(
        DOM.addDisposableListener(element, DOM.EventType.CONTEXT_MENU, (e) => {
    ...
    this._register(
      DOM.addDisposableListener(element, DOM.EventType.CLICK, (e) => {
    ...
    this._register(
      DOM.addDisposableListener(element, DOM.EventType.DBLCLICK, (e) => {
        DOM.EventHelper.stop(e, true);
      })
    );
    [DOM.EventType.MOUSE_UP, DOM.EventType.MOUSE_OUT].forEach((event) => {
      this._register(
        DOM.addDisposableListener(element, event, (e) => {
  ...
  onClick(event) {
    ...
    const context = types.isUndefinedOrNull(this._context)
      ? ((_a = this.options) === null || _a === void 0 ? void 0 : _a.useEventAsContext)
        ? event
        : undefined
      : this._context;
