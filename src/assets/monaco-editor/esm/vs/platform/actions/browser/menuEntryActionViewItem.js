...
let MenuEntryActionViewItem = class MenuEntryActionViewItem extends ActionViewItem {
  ...
  _updateItemClass(item) {
    ...
    const icon =
      this._commandAction.checked && ((_a = item.toggled) === null || _a === void 0 ? void 0 : _a.icon)
        ? item.toggled.icon
        : item.icon;
      ...
      if ((_b = icon.dark) === null || _b === void 0 ? void 0 : _b.scheme) {
        ...
        if (ICON_PATH_TO_CSS_RULES.has(iconPathMapKey)) {
          ...
          createCSSRule(
            ...
            `background-image: ${asCSSUrl(icon.dark)}`
          );
...
MenuEntryActionViewItem = __decorate(
  [__param(1, IKeybindingService), __param(2, INotificationService), __param(3, IContextMenuService)],
    ...
    if (action.item.icon) {
      if (ThemeIcon.isThemeIcon(action.item.icon)) {
        ...
        if (ICON_PATH_TO_CSS_RULES.has(iconPathMapKey)) {
          ...
          createCSSRule(
            `.icon.${className}`,
            `background-image: ${asCSSUrl(action.item.icon.light || action.item.icon.dark)}`
          );
          createCSSRule(
            `.vs-dark .icon.${className}, .hc-black .icon.${className}`,
            `background-image: ${asCSSUrl(action.item.icon.dark)}`
          );
...
SubmenuEntryActionViewItem = __decorate(
  [__param(1, INotificationService), __param(2, IContextMenuService)],
