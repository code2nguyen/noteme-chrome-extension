...
let MarkerDecorationsService = class MarkerDecorationsService extends Disposable {
  ...
  _createDecorationRange(model, rawMarker) {
    ...
    if (
      ...
      !this._hasMarkerTag(rawMarker, 1 /* Unnecessary */) &&
      !this._hasMarkerTag(rawMarker, 2 /* Deprecated */)
    ) {
    ...
    if (ret.isEmpty()) {
      ...
      if (word) {
        ...
        let maxColumn =
          model.getLineLastNonWhitespaceColumn(ret.startLineNumber) || model.getLineMaxColumn(ret.startLineNumber);
        if (maxColumn === 1) {
          // empty line
...
MarkerDecorationsService = __decorate(
  [__param(0, IModelService), __param(1, IMarkerService)],
