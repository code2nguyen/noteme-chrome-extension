...
export class MonospaceLineBreaksComputerFactory {
  ...
  static create(options) {
    return new MonospaceLineBreaksComputerFactory(
      options.get(108 /* wordWrapBreakBeforeCharacters */),
      options.get(107 /* wordWrapBreakAfterCharacters */)
    );
  }
    ...
    return {
      ...
      finalize: () => {
        const columnsForFullWidthChar =
          fontInfo.typicalFullwidthCharacterWidth / fontInfo.typicalHalfwidthCharacterWidth; //@perf
        let result = [];
...
function createLineBreaksFromPreviousLineBreaks(
  ...
  while (prevIndex < prevLen) {
    ...
    while (
      prevIndex < 0 ||
      (prevIndex < prevLen && prevBreakingOffsetsVisibleColumn[prevIndex] < breakOffsetVisibleColumn)
    ) {
      prevIndex++;
    }
    let bestDistance = Math.abs(prevBreakingOffsetsVisibleColumn[prevIndex] - breakingColumn);
  ...
  if (wrappingIndent !== 0 /* None */) {
    ...
    if (firstNonWhitespaceIndex !== -1) {
      ...
      for (let i = 0; i < firstNonWhitespaceIndex; i++) {
        const charWidth =
          lineText.charCodeAt(i) === 9 /* Tab */ ? tabCharacterWidth(wrappedTextIndentLength, tabSize) : 1;
        wrappedTextIndentLength += charWidth;
      }
      // Increase indent of continuation lines, if desired
      const numberOfAdditionalTabs =
        wrappingIndent === 3 /* DeepIndent */ ? 2 : wrappingIndent === 2 /* Indent */ ? 1 : 0;
