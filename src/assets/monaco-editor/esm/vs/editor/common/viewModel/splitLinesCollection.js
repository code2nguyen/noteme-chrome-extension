...
export class SplitLinesCollection {
  ...
  setHiddenAreas(_ranges) {
    ...
    let nextLineNumberToUpdateHiddenArea =
      hiddenAreaIdx + 1 < hiddenAreas.length ? hiddenAreaEnd + 1 : this.lines.length + 2;
  ...
  setWrappingSettings(fontInfo, wrappingStrategy, wrappingColumn, wrappingIndent) {
    ...
    const onlyWrappingColumnChanged =
      equalFontInfo && equalWrappingStrategy && !equalWrappingColumn && equalWrappingIndent;
    this.fontInfo = fontInfo;
    this.wrappingStrategy = wrappingStrategy;
    this.wrappingColumn = wrappingColumn;
    this.wrappingIndent = wrappingIndent;
  ...
  onModelLinesDeleted(versionId, fromLineNumber, toLineNumber) {
    ...
    let outputFromLineNumber =
      fromLineNumber === 1 ? 1 : this.prefixSumComputer.getAccumulatedValue(fromLineNumber - 2) + 1;
  ...
  onModelLineChanged(versionId, lineNumber, lineBreakData) {
    ...
    const viewLinesChangedEvent =
      changeFrom <= changeTo ? new viewEvents.ViewLinesChangedEvent(changeFrom, changeTo) : null;
    const viewLinesInsertedEvent =
      insertFrom <= insertTo ? new viewEvents.ViewLinesInsertedEvent(insertFrom, insertTo) : null;
    const viewLinesDeletedEvent =
      deleteFrom <= deleteTo ? new viewEvents.ViewLinesDeletedEvent(deleteFrom, deleteTo) : null;
  ...
  getDecorationsInRange(range, ownerId, filterOutValidation) {
    ...
    if (modelEnd.lineNumber - modelStart.lineNumber <= range.endLineNumber - range.startLineNumber) {
      ...
      return this.model.getDecorationsInRange(
        new Range(modelStart.lineNumber, 1, modelEnd.lineNumber, modelEnd.column),
        ownerId,
        filterOutValidation
      );
    }
        ...
        if (reqStart !== null) {
          ...
          result = result.concat(
            this.model.getDecorationsInRange(
              new Range(reqStart.lineNumber, reqStart.column, modelLineIndex, maxLineColumn),
              ownerId,
              filterOutValidation
            )
          );
    ...
    if (reqStart !== null) {
      result = result.concat(
        this.model.getDecorationsInRange(
          new Range(reqStart.lineNumber, reqStart.column, modelEnd.lineNumber, modelEnd.column),
          ownerId,
          filterOutValidation
        )
      );
...
export class SplitLine {
  ...
  getViewLineData(model, modelLineNumber, outputLineIndex) {
    ...
    const startVisibleColumn =
      outputLineIndex === 0 ? 0 : this._lineBreakData.breakOffsetsVisibleColumn[outputLineIndex - 1];
  ...
  getModelColumnOfViewPosition(outputLineIndex, outputColumn) {
    ...
    return (
      LineBreakData.getInputOffsetOfOutputPosition(this._lineBreakData.breakOffsets, outputLineIndex, adjustedColumn) +
      1
    );
  }
  getViewPositionOfModelPosition(deltaLineNumber, inputColumn) {
