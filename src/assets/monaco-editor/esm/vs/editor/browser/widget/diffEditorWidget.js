...
let DiffEditorWidget = class DiffEditorWidget extends Disposable {
  constructor(
    ...
    this._register(
      dom.addStandardDisposableListener(this._overviewDomElement, 'mousedown', (e) => {
    ...
    this._elementSizeObserver = this._register(
      new ElementSizeObserver(this._containerDomElement, undefined, () => this._onDidContainerSizeChanged())
    );
    ...
    this.modifiedEditor = this._createRightHandSideEditor(
      options,
  ...
  _createLeftHandSideEditor(options, instantiationService, contextKeyService) {
    const editor = this._createInnerEditor(
      ...
      this._adjustOptionsForLeftHandSide(options, this._originalIsEditable, this._originalCodeLens)
    );
    this._register(
    ...
    this._register(
      editor.onDidContentSizeChange((e) => {
        const width =
          this.originalEditor.getContentWidth() +
          this.modifiedEditor.getContentWidth() +
  ...
  _createRightHandSideEditor(options, instantiationService, contextKeyService) {
    const editor = this._createInnerEditor(
      ...
      this._adjustOptionsForRightHandSide(options, this._modifiedCodeLens)
    );
    this._register(
      editor.onDidScrollChange((e) => {
        if (this._isHandlingScrollEvent) {
  ...
  updateOptions(newOptions) {
    ...
    this.originalEditor.updateOptions(
      this._adjustOptionsForLeftHandSide(newOptions, this._originalIsEditable, this._originalCodeLens)
    );
    // enableSplitViewResizing
    ...
    if (!this._editorWorkerService.canComputeDiff(currentOriginalModel.uri, currentModifiedModel.uri)) {
      if (
        ...
        this._notificationService.warn(
          nls.localize('diff.tooLarge', 'Cannot compare files because one file is too large.')
        );
      }
      return;
    }
  ...
  _adjustOptionsForRightHandSide(options, isCodeLensEnabled) {
    ...
    result.revealHorizontalRightPadding =
      EditorOptions.revealHorizontalRightPadding.defaultValue + DiffEditorWidget.ENTIRE_DIFF_OVERVIEW_WIDTH;
  ...
  _getLineChangeAtOrBeforeLineNumber(lineNumber, startLineNumberExtractor) {
    ...
    while (min < max) {
      ...
      let midEnd =
        mid + 1 <= max ? startLineNumberExtractor(lineChanges[mid + 1]) : 1073741824 /* MAX_SAFE_SMALL_INTEGER */;
  ...
  _getEquivalentLineForOriginalLineNumber(lineNumber) {
    ...
    let originalEquivalentLineNumber =
      lineChange.originalStartLineNumber + (lineChange.originalEndLineNumber > 0 ? -1 : 0);
    let modifiedEquivalentLineNumber =
      lineChange.modifiedStartLineNumber + (lineChange.modifiedEndLineNumber > 0 ? -1 : 0);
    let lineChangeOriginalLength =
      lineChange.originalEndLineNumber > 0
        ? lineChange.originalEndLineNumber - lineChange.originalStartLineNumber + 1
        : 0;
    let lineChangeModifiedLength =
  ...
  getViewZones() {
    ...
    for (let i = 0, length = this.lineChanges.length; i <= length; i++) {
      ...
      while (
        ...
        modifiedForeignVZ.current.afterLineNumber <= modifiedEndEquivalentLineNumber
      ) {
        let viewZoneLineNumber;
        if (modifiedForeignVZ.current.afterLineNumber <= modifiedEquivalentLineNumber) {
          viewZoneLineNumber =
            originalEquivalentLineNumber - modifiedEquivalentLineNumber + modifiedForeignVZ.current.afterLineNumber;
        } else {
        ...
        if (
          lineChange &&
          lineChange.modifiedStartLineNumber <= modifiedForeignVZ.current.afterLineNumber &&
          modifiedForeignVZ.current.afterLineNumber <= lineChange.modifiedEndLineNumber
        ) {
      ...
      while (
        ...
        originalForeignVZ.current.afterLineNumber <= originalEndEquivalentLineNumber
      ) {
        let viewZoneLineNumber;
        if (originalForeignVZ.current.afterLineNumber <= originalEquivalentLineNumber) {
          viewZoneLineNumber =
            modifiedEquivalentLineNumber - originalEquivalentLineNumber + originalForeignVZ.current.afterLineNumber;
        } else {
...
export function createDecoration(startLineNumber, startColumn, endLineNumber, endColumn, options) {
  return {
...
export class DiffEditorWidgetSideBySide extends DiffEditorWidgetStyle {
  constructor(dataSource, enableSplitViewResizing) {
    ...
    this._sash = this._register(
      new Sash(this._dataSource.getContainerDomNode(), this, { orientation: 0 /* VERTICAL */ })
    );
  ...
  _getViewZones(lineChanges, originalForeignVZ, modifiedForeignVZ, originalEditor, modifiedEditor) {
    let c = new SideBySideViewZonesComputer(
      lineChanges,
      originalForeignVZ,
      originalEditor.getOption(51 /* lineHeight */),
      modifiedForeignVZ,
      modifiedEditor.getOption(51 /* lineHeight */)
    );
    ...
    for (let i = 0, length = lineChanges.length; i < length; i++) {
      ...
      if (isChangeOrDelete(lineChange)) {
        ...
        result.overviewZones.push(
          new OverviewRulerZone(lineChange.originalStartLineNumber, lineChange.originalEndLineNumber, overviewZoneColor)
        );
            ...
            if (isChangeOrDelete(charChange)) {
              if (ignoreTrimWhitespace) {
                for (
                  ...
                  result.decorations.push(
                    createDecoration(lineNumber, startColumn, lineNumber, endColumn, DECORATIONS.charDelete)
                  );
                }
              } else {
                result.decorations.push(
  ...
  _getModifiedEditorDecorations(lineChanges, ignoreTrimWhitespace, renderIndicators, originalEditor, modifiedEditor) {
    ...
    for (let i = 0, length = lineChanges.length; i < length; i++) {
      ...
      if (isChangeOrInsert(lineChange)) {
        ...
        result.overviewZones.push(
          new OverviewRulerZone(lineChange.modifiedStartLineNumber, lineChange.modifiedEndLineNumber, overviewZoneColor)
        );
        if (lineChange.charChanges) {
          for (let j = 0, lengthJ = lineChange.charChanges.length; j < lengthJ; j++) {
            let charChange = lineChange.charChanges[j];
            if (isChangeOrInsert(charChange)) {
              if (ignoreTrimWhitespace) {
                for (
                  ...
                  result.decorations.push(
                    createDecoration(lineNumber, startColumn, lineNumber, endColumn, DECORATIONS.charInsert)
                  );
                }
              } else {
                result.decorations.push(
...
class DiffEditorWidgetInline extends DiffEditorWidgetStyle {
  constructor(dataSource, enableSplitViewResizing) {
    ...
    this._register(
      dataSource.getOriginalEditor().onDidLayoutChange((layoutInfo) => {
...
class InlineViewZonesComputer extends ViewZonesComputer {
    ...
    for (
      ...
      if (this.renderIndicators) {
        ...
        marginHTML = marginHTML.concat([
          `<div class="delete-sign ${diffRemoveIcon.classNames}" style="position:absolute;top:${
            index * lineHeight
          }px;width:${lineDecorationsWidth}px;height:${lineHeight}px;right:0;"></div>`,
        ]);
      }
    }
  ...
  _renderOriginalLine(count, originalModel, options, tabSize, lineNumber, decorations, sb) {
    ...
    const output = renderViewLine(
      new RenderLineInput(
        fontInfo.isMonospace && !options.get(24 /* disableMonospaceOptimizations */),
        ...
        options.get(95 /* stopRenderingLineAfter */),
        options.get(80 /* renderWhitespace */),
        options.get(74 /* renderControlCharacters */),
        options.get(37 /* fontLigatures */) !== EditorFontLigatures.OFF,
...
export function isChangeOrInsert(lineChange) {
  return lineChange.modifiedEndLineNumber > 0;
}
export function isChangeOrDelete(lineChange) {
