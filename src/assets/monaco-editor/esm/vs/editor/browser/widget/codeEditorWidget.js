...
let CodeEditorWidget = class CodeEditorWidget extends Disposable {
  ...
  getConfiguredWordAtPosition(position) {
    ...
    return WordOperations.getWordAtPosition(
      ...
      this._configuration.options.get(105 /* wordSeparators */),
      position
    );
  }
  ...
  getScrolledVisiblePosition(rawPosition) {
    ...
    const layoutInfo = options.get(117 /* layoutInfo */);
    const top =
      CodeEditorWidget._getVerticalOffsetForPosition(this._modelData, position.lineNumber, position.column) -
      this.getScrollTop();
    const left =
      this._modelData.view.getOffsetForColumn(position.lineNumber, position.column) +
  ...
  _attachModel(model) {
    ...
    listenersToRemove.push(
      model.onDidChangeLanguageConfiguration((e) => this._onDidChangeModelLanguageConfiguration.fire(e))
    );
...
registerThemingParticipant((theme, collector) => {
  ...
  if (infoBorderColor) {
    collector.addRule(
      `.monaco-editor .${'squiggly-info' /* EditorInfoDecoration */} { border-bottom: 4px double ${infoBorderColor}; }`
    );
  }
  ...
  if (hintBorderColor) {
    collector.addRule(
      `.monaco-editor .${'squiggly-hint' /* EditorHintDecoration */} { border-bottom: 2px dotted ${hintBorderColor}; }`
    );
  }
  ...
  if (unnecessaryForeground) {
    collector.addRule(
      `.monaco-editor.showUnused .${'squiggly-inline-unnecessary' /* EditorUnnecessaryInlineDecoration */} { opacity: ${
