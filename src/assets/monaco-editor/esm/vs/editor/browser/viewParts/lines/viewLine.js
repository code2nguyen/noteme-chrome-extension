...
export class ViewLineOptions {
  constructor(config, themeType) {
    ...
    this.useMonospaceOptimizations = fontInfo.isMonospace && !options.get(24 /* disableMonospaceOptimizations */);
...
export class ViewLine {
  ...
  onSelectionChanged() {
    if (
      ...
      this._options.themeType === HIGH_CONTRAST ||
  ...
  renderLine(lineNumber, deltaTop, viewportData, sb) {
    ...
    if (
      alwaysRenderInlineSelection ||
      options.themeType === HIGH_CONTRAST ||
      this._options.renderWhitespace === 'selection'
    ) {
        ...
        if (startColumn < endColumn) {
          if (options.themeType === HIGH_CONTRAST || this._options.renderWhitespace !== 'selection') {
            actualInlineDecorations.push(
              new LineDecoration(startColumn, endColumn, 'inline-selected-text', 0 /* Regular */)
            );
          } else {
            if (!selectionsOnLine) {
              selectionsOnLine = [];
    ...
    if (
      ...
      lineData.isBasicASCII &&
      options.useMonospaceOptimizations &&
      output.containsForeignElements === 0 /* None */
    ) {
...
class RenderedViewLine {
  ...
  _actualReadPixelOffset(domNode, column, context) {
    ...
    if (
      ...
      this._containsForeignElements === 0 /* None */
    ) {
