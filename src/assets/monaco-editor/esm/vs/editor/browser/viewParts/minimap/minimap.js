  ...
  static create(
    ...
    if (options.minimapHeightIsEditorHeight) {
      const logicalScrollHeight =
        realLineCount * options.lineHeight + (options.scrollBeyondLastLine ? viewportHeight - options.lineHeight : 0);
    ...
    if (minimapLinesFitting >= lineCount + extraLinesAtTheBottom) {
      ...
      let startLineNumber = Math.max(
        1,
        Math.floor(viewportStartLineNumber - (sliderTop * pixelRatio) / minimapLineHeight)
      );
      ...
      if (this._samplingState) {
        ...
        for (const selection of this._selections) {
          ...
          this._minimapSelections.push(
            new Selection(minimapLineStart, selection.startColumn, minimapLineEnd, selection.endColumn)
          );
        }
      } else {
      ...
      for (const decoration of decorations) {
        ...
        result.push(
          new ViewModelDecoration(
            new Range(minimapStartLineNumber, range.startColumn, minimapEndLineNumber, range.endColumn),
            decoration.options
          )
        );
      }
...
class InnerMinimap extends Disposable {
  constructor(theme, model) {
    ...
    this._mouseDownListener = dom.addStandardDisposableListener(this._domNode.domNode, 'mousedown', (e) => {
      ...
      const internalOffsetY =
        (this._model.options.canvasInnerHeight / this._model.options.canvasOuterHeight) * e.browserEvent.offsetY;
  ...
  _startSliderDragging(initialButtons, initialPosX, initialPosY, posy, initialSliderState) {
    ...
    this._sliderMouseMoveMonitor.startMonitoring(
      ...
      (mouseMoveData) => handleMouseMove(mouseMoveData.posy, mouseMoveData.posx),
      () => {
        this._slider.toggleClassName('active', false);
      }
    );
  ...
  render(renderingCtx) {
    ...
    const horizontalSliderLeft = Math.min(
      ...
      Math.round((scrollLeftChars * this._model.options.minimapCharWidth) / this._model.options.pixelRatio)
    );
  ...
  renderLines(layout) {
    ...
    const innerLinePadding =
      minimapLineHeight > renderMinimapLineHeight ? Math.floor((minimapLineHeight - renderMinimapLineHeight) / 2) : 0;
