...
class InspectTokensWidget extends Disposable {
  ...
  _compute(position) {
    ...
    if (token1Index < data.tokens1.length) {
      ...
      let tokenEndIndex =
        token1Index + 1 < data.tokens1.length ? data.tokens1[token1Index + 1].offset : lineContent.length;
    ...
    const metadata =
      (token2Index << 1) + 1 < data.tokens2.length ? this._decodeMetadata(data.tokens2[(token2Index << 1) + 1]) : null;
    append(
      this._domNode,
      $(
