...
let SnippetController2 = class SnippetController2 {
  ...
  insert(template, opts) {
    ...
    try {
      this._doInsert(
        template,
        typeof opts === 'undefined' ? _defaultOptions : Object.assign(Object.assign({}, _defaultOptions), opts)
      );
    } catch (e) {
      this.cancel();
  ...
  _handleChoice() {
    ...
    if (this._currentChoice !== choice) {
      ...
      this._editor.setSelections(
        this._editor.getSelections().map((s) => Selection.fromPositions(s.getStartPosition()))
      );
