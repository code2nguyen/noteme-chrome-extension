...
export class Scanner {
  ...
  static isVariableCharacter(ch) {
    return (
      ch === 95 /* Underline */ || (ch >= 97 /* a */ && ch <= 122) /* z */ || (ch >= 65 /* A */ && ch <= 90) /* Z */
    );
  }
  text(value) {
    this.value = value;
...
export class SnippetParser {
  ...
  _parseComplexPlaceholder(parent) {
    ...
    const token = this._token;
    const match =
      this._accept(0 /* Dollar */) && this._accept(3 /* CurlyOpen */) && (index = this._accept(8 /* Int */, true));
    if (!match) {
      return this._backTo(token);
    }
