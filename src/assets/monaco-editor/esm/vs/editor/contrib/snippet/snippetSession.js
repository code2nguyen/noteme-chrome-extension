...
registerThemingParticipant((theme, collector) => {
  ...
  collector.addRule(
    ...
    )}; outline-color: ${getColorGraceful(colors.snippetTabstopHighlightBorder)}; }`
  );
  collector.addRule(
    `.monaco-editor .finish-snippet-placeholder { background-color: ${getColorGraceful(
      colors.snippetFinalTabstopHighlightBackground
    )}; outline-color: ${getColorGraceful(colors.snippetFinalTabstopHighlightBorder)}; }`
  );
});
export class OneSnippet {
  ...
  move(fwd) {
    ...
    return !couldSkipThisPlaceholder
      ? newSelections !== null && newSelections !== void 0
        ? newSelections
        : []
      : this.move(fwd);
  }
...
export class SnippetSession {
      ...
      if (marker instanceof Text && !(marker.parent instanceof Choice)) {
        ...
        if (adjustIndentation) {
          for (let i = 1; i < lines.length; i++) {
            ...
            lines[i] =
              model.normalizeIndentation(lineLeadingWhitespace + templateLeadingWhitespace) +
  ...
  static createEditsAndSnippets(
    ...
    const modelBasedVariableResolver = editor.invokeWithinContext(
      (accessor) => new ModelBasedVariableResolver(accessor.get(ILabelService, optional), model)
    );
    ...
    for (const { selection, idx } of indexedSelections) {
      ...
      SnippetSession.adjustWhitespace(
        ...
        adjustWhitespace ||
          (idx > 0 &&
            firstLineFirstNonWhitespace !== model.getLineFirstNonWhitespaceColumn(selection.positionLineNumber)),
        true
      );
      snippet.resolveVariables(
        new CompositeSnippetVariableResolver([
          ...
          new ClipboardBasedVariableResolver(
            ...
            editor.getOption(62 /* multiCursorPaste */) === 'spread'
          ),
