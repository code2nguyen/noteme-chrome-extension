...
let OnTypeRenameContribution = class OnTypeRenameContribution extends Disposable {
  ...
  reinitialize() {
    ...
    const isEnabled =
      model !== null && this._editor.getOption(73 /* renameOnType */) && OnTypeRenameProviderRegistry.has(model);
        ...
        __awaiter(this, void 0, void 0, function* () {
          try {
            ...
            this._currentWordPattern =
              (response === null || response === void 0 ? void 0 : response.wordPattern) || this._languageWordPattern;
...
OnTypeRenameContribution = __decorate([__param(1, IContextKeyService)], OnTypeRenameContribution);
...
export function getOnTypeRenameRanges(model, position, token) {
  ...
  return first(
    orderedByScore.map((provider) => () => {
      return Promise.resolve(provider.provideOnTypeRenameRanges(model, position, token)).then(
        (res) => {
          if (!res) {
            return undefined;
          }
...
export const editorOnTypeRenameBackground = registerColor(
  ...
  nls.localize('editorOnTypeRenameBackground', 'Background color when the editor auto renames on type.')
);
registerThemingParticipant((theme, collector) => {
  ...
  if (editorOnTypeRenameBackgroundColor) {
    collector.addRule(
      `.monaco-editor .on-type-rename-decoration { background: ${editorOnTypeRenameBackgroundColor}; border-left-color: ${editorOnTypeRenameBackgroundColor}; }`
    );
  }
});
registerModelAndPositionCommand('_executeRenameOnTypeProvider', (model, position) =>
  getOnTypeRenameRanges(model, position, CancellationToken.None)
);
