...
const editorWordHighlightStrong = registerColor(
...
const editorWordHighlightBorder = registerColor(
...
const editorWordHighlightStrongBorder = registerColor(
...
const overviewRulerWordHighlightStrongForeground = registerColor(
  ...
  { dark: '#C0A0C0CC', light: '#C0A0C0CC', hc: '#C0A0C0CC' },
  nls.localize(
...
export function getOccurrencesAtPosition(model, position, token) {
  ...
  return first(
    orderedByScore.map((provider) => () => {
      return Promise.resolve(provider.provideDocumentHighlights(model, position, token)).then(
        undefined,
...
registerModelAndPositionCommand('_executeDocumentHighlights', (model, position) =>
  getOccurrencesAtPosition(model, position, CancellationToken.None)
);
class WordHighlighter {
  ...
  _getSortedHighlights() {
    return arrays.coalesce(
      this._decorationIds.map((id) => this.model.getDecorationRange(id)).sort(Range.compareRangesUsingStarts)
    );
  }
  moveNext() {
  ...
  _run() {
    ...
    const workerRequestIsValid =
      this.workerRequest && this.workerRequest.isValid(this.model, editorSelection, this._decorationIds);
    // There are 4 cases:
    ...
    if (workerRequestIsValid) {
      ...
      this.workerRequestCompleted = false;
      this.workerRequest = computeOccurencesAtPosition(
        this.model,
        this.editor.getSelection(),
        this.editor.getOption(105 /* wordSeparators */)
      );
