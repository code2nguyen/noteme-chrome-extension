...
export class LineCommentCommand {
    ...
    for (let i = 0, lineCount = lines.length; i < lineCount; i++) {
      ...
      if (shouldRemoveComments && insertSpace) {
        ...
        if (
          ...
          lineContent.charCodeAt(commentStrEndOffset) === 32 /* Space */
        ) {
  ...
  _attemptRemoveBlockComment(model, s, startToken, endToken) {
    ...
    if (
      startTokenIndex !== -1 &&
      model.getLineContent(startLineNumber).charCodeAt(startTokenIndex + startToken.length) === 32 /* Space */
    ) {
      startToken += ' ';
    }
    ...
    for (let i = 0, len = lines.length; i < len; i++) {
      ...
      for (let j = 0, lenJ = lines[i].commentStrOffset; currentVisibleColumn < minVisibleColumn && j < lenJ; j++) {
        currentVisibleColumn = LineCommentCommand.nextVisibleColumn(
          ...
          tabSize,
          lineContent.charCodeAt(j) === 9 /* Tab */,
          1
        );
      }
