...
export class BlockCommentCommand {
  ...
  _createOperationsForBlockComment(selection, startToken, endToken, insertSpace, model, builder) {
    ...
    if (startTokenIndex !== -1 && endTokenIndex !== -1) {
      ...
      if (
        ...
        startLineText.charCodeAt(startTokenIndex + startToken.length) === 32 /* Space */
      ) {
    ...
    if (!Range.isEmpty(r)) {
      ...
      res.push(
        EditOperation.delete(
          new Range(r.startLineNumber, r.startColumn - startToken.length, r.startLineNumber, r.startColumn)
        )
      );
      // Remove block comment end
      res.push(
        EditOperation.delete(new Range(r.endLineNumber, r.endColumn, r.endLineNumber, r.endColumn + endToken.length))
      );
    } else {
  ...
  static _createAddBlockCommentOperations(r, startToken, endToken, insertSpace) {
    ...
    if (!Range.isEmpty(r)) {
      ...
      res.push(
        EditOperation.insert(new Position(r.startLineNumber, r.startColumn), startToken + (insertSpace ? ' ' : ''))
      );
