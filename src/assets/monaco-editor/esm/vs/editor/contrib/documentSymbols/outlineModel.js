...
export class OutlineModel extends TreeElement {
  ...
  static _create(textModel, token) {
    ...
    const promises = provider.map((provider, index) => {
      ...
      let group = new OutlineGroup(
        id,
        result,
        (_a = provider.displayName) !== null && _a !== void 0 ? _a : 'Unknown Outline Provider',
        index
      );
...
OutlineModel._keys = new (class {
  ...
  for(textModel, version) {
    return `${textModel.id}/${version ? textModel.getVersionId() : ''}/${this._hash(
      DocumentSymbolProviderRegistry.all(textModel)
    )}`;
  }
  _hash(providers) {
    let result = '';
