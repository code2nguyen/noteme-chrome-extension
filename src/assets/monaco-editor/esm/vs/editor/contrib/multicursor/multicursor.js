...
class InsertCursorAtEndOfEachLineSelected extends EditorAction {
  ...
  getCursorsForSelection(selection, model, result) {
    ...
    if (selection.endColumn > 1) {
      result.push(
        new Selection(selection.endLineNumber, selection.endColumn, selection.endLineNumber, selection.endColumn)
      );
    }
  }
  run(accessor, editor) {
  ...
  _getNextMatch() {
    ...
    const nextMatch = this._editor
      ...
      .findNextMatch(
        ...
        false,
        this.matchCase,
        this.wholeWord ? this._editor.getOption(105 /* wordSeparators */) : null,
        false
      );
    if (!nextMatch) {
      return null;
...
export class MultiCursorSelectionController extends Disposable {
  ...
  selectAll(findController) {
    ...
    if (findState.isRevealed && findState.searchString.length > 0 && findState.isRegex) {
      matches = this._editor
        ...
        .findMatches(
          ...
          findState.isRegex,
          findState.matchCase,
          findState.wholeWord ? this._editor.getOption(105 /* wordSeparators */) : null,
          false,
          1073741824 /* MAX_SAFE_SMALL_INTEGER */
        );
    } else {
    ...
    if (findState.searchScope) {
      ...
      matches.forEach((match) => {
        states.forEach((state) => {
          if (
            match.range.endLineNumber <= state.endLineNumber &&
            match.range.startLineNumber >= state.startLineNumber
          ) {
    ...
    if (matches.length > 0) {
      ...
      this._setSelections(
        matches.map(
          (m) => new Selection(m.range.startLineNumber, m.range.startColumn, m.range.endLineNumber, m.range.endColumn)
        )
      );
    }
  }
...
export class SelectionHighlighter extends Disposable {
  ...
  static _createState(isEnabled, editor) {
    ...
    return new SelectionHighlighterState(
      r.searchText,
      r.matchCase,
      r.wholeWord ? editor.getOption(105 /* wordSeparators */) : null,
  ...
  _setState(state) {
    ...
    const hasFindOccurrences =
      DocumentHighlightProviderRegistry.has(model) && this.editor.getOption(63 /* occurrencesHighlight */);
    let allMatches = model
      .findMatches(this.state.searchText, true, false, this.state.matchCase, this.state.wordSeparators, false)
      .map((m) => m.range);
