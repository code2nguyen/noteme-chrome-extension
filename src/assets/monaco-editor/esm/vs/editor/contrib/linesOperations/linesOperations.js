...
export class InsertLineBeforeAction extends EditorAction {
  ...
  run(_accessor, editor) {
    ...
    editor.executeCommands(
      this.id,
      TypeOperations.lineInsertBefore(viewModel.cursorConfig, editor.getModel(), editor.getSelections())
    );
  }
}
export class InsertLineAfterAction extends EditorAction {
  ...
  run(_accessor, editor) {
    ...
    editor.executeCommands(
      this.id,
      TypeOperations.lineInsertAfter(viewModel.cursorConfig, editor.getModel(), editor.getSelections())
    );
  }
}
  ...
  run(_accessor, editor) {
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      if (cursor.column >= maxColumn) {
        ...
        commands.push(
          new ReplaceCommand(
            new Selection(cursor.lineNumber, Math.max(1, cursor.column - 1), cursor.lineNumber + 1, 1),
            chars
          )
        );
      } else {
  ...
  run(_accessor, editor) {
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      if (selection.isEmpty()) {
        ...
        commands.push(
          new ReplaceCommandThatPreservesSelection(selection, this._modifyText(text, wordSeparators), selection)
        );
      }
    }
    editor.pushUndoStop();
