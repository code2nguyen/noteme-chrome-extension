...
export class ModesContentHoverWidget extends ContentHoverWidget {
  constructor(
    ...
    this._hoverOperation = new HoverOperation(
      ...
      this._editor.getOption(46 /* hover */).delay
    );
    this._register(
      dom.addStandardDisposableListener(this.getDomNode(), dom.EventType.FOCUS, () => {
    ...
    this._register(
      dom.addStandardDisposableListener(this.getDomNode(), dom.EventType.BLUR, () => {
  ...
  _renderMessages(renderRange, messages) {
    ...
    messages.forEach((msg) => {
      ...
      if (msg instanceof ColorHover) {
        ...
        const widget = new ColorPickerWidget(
          fragment,
          model,
          this._editor.getOption(115 /* pixelRatio */),
          this._themeService
        );
        getColorPresentations(editorModel, colorInfo, msg.provider, CancellationToken.None).then(
          (colorPresentations) => {
            ...
            const updateEditorModel = () => {
              ...
              if (model.presentation.textEdit) {
                ...
                newRange = newRange.setEndPosition(
                  newRange.endLineNumber,
                  newRange.startColumn + model.presentation.textEdit.text.length
                );
              } else {
          ...
          msg.contents
            ...
            .forEach((contents) => {
              ...
              const renderer = markdownDisposeables.add(
                new MarkdownRenderer(this._editor, this._modeService, this._openerService)
              );
              markdownDisposeables.add(
                renderer.onDidRenderCodeBlock(() => {
    ...
    if (markerMessages.length) {
      ...
      const markerHoverForStatusbar =
        markerMessages.length === 1
          ? markerMessages[0]
          : markerMessages.sort((a, b) => MarkerSeverity.compare(a.marker.severity, b.marker.severity))[0];
      ...
      for (const { message, resource, startLineNumber, startColumn } of relatedInformation) {
        ...
        a.onclick = (e) => {
          ...
          if (this._openerService) {
            this._openerService
              .open(resource.with({ fragment: `${startLineNumber},${startColumn}` }), { fromUserGesture: true })
  ...
  renderMarkerStatusbar(markerHover) {
    ...
    if (
      ...
      disposables.add(
        this._renderAction(actionsElement, {
