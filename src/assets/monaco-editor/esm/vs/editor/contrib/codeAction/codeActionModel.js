...
export class CodeActionModel extends Disposable {
  ...
  _update() {
    ...
    const model = this._editor.getModel();
    if (model && CodeActionProviderRegistry.has(model) && !this._editor.getOption(72 /* readOnly */)) {
      ...
      this._codeActionOracle.value = new CodeActionOracle(
        ...
        (trigger) => {
          ...
          const actions = createCancelablePromise((token) =>
            getCodeActions(model, trigger.selection, trigger.trigger, Progress.None, token)
          );
