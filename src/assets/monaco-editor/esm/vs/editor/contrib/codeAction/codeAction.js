...
class ManagedCodeActionSet extends Disposable {
  ...
  get hasAutoFix() {
    return this.validActions.some(
      (fix) => !!fix.kind && CodeActionKind.QuickFix.contains(new CodeActionKind(fix.kind)) && !!fix.isPreferred
    );
  }
}
  ...
  const promises = providers.map((provider) =>
    __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        const providedCodeActions = yield provider.provideCodeActions(
          model,
          rangeOrSelection,
          codeActionContext,
          cts.token
        ...
        const filteredActions = (
          (providedCodeActions === null || providedCodeActions === void 0 ? void 0 : providedCodeActions.actions) || []
        ).filter((action) => action && filtersAction(filter, action));
...
function getCodeActionProviders(model, filter) {
  return (
    modes.CodeActionProviderRegistry.all(model)
