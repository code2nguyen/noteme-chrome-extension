  ...
  constructor(
    ...
    this.widget = this._toDispose.add(
      new IdleValue(() => {
        ...
        const commitCharacterController = new CommitCharacterController(this.editor, widget, (item) =>
          this._insertSuggestion(item, 2 /* NoAfterUndoStop */)
        );
        ...
        this._toDispose.add(
          widget.onDidFocus(({ item }) => {
            ...
            let value = true;
            if (
              this.editor.getOption(1 /* acceptSuggestionOnEnter */) === 'smart' &&
        ...
        this._toDispose.add(
          widget.onDetailsKeyDown((e) => {
            ...
            if (
              e.toKeybinding().equals(new SimpleKeybinding(true, false, false, false, 33 /* KEY_C */)) ||
              (platform.isMacintosh &&
                e.toKeybinding().equals(new SimpleKeybinding(false, false, false, true, 33 /* KEY_C */)))
            ) {
              e.stopPropagation();
              return;
            }
      ...
      this._alternatives.value.set(event, (next) => {
        ...
        while (model.canUndo()) {
          ...
          this._insertSuggestion(
            ...
            1 /* NoBeforeUndoStop */ |
              2 /* NoAfterUndoStop */ |
              (flags & 8 /* AlternativeOverwriteConfig */ ? 8 /* AlternativeOverwriteConfig */ : 0)
          );
          break;
        }
      });
  ...
  _alertCompletionItem({ completion: suggestion }) {
    ...
    if (isNonEmptyArray(suggestion.additionalTextEdits)) {
      let msg = nls.localize(
        ...
        textLabel,
        suggestion.additionalTextEdits.length
      );
      alert(msg);
    }
  }
...
MenuRegistry.appendMenuItem(suggestWidgetStatusbarMenu, {
  command: {
    id: 'acceptSelectedSuggestion',
    title: nls.localize(
      { key: 'accept.accept', comment: ['{0} will be a keybinding, e.g "Enter to insert"'] },
      '{0} to insert'
    ),
  },
  group: 'left',
...
MenuRegistry.appendMenuItem(suggestWidgetStatusbarMenu, {
  command: {
    id: 'acceptSelectedSuggestion',
    title: nls.localize(
      { key: 'accept.insert', comment: ['{0} will be a keybinding, e.g "Enter to insert"'] },
      '{0} to insert'
    ),
  },
  group: 'left',
...
MenuRegistry.appendMenuItem(suggestWidgetStatusbarMenu, {
  command: {
    id: 'acceptSelectedSuggestion',
    title: nls.localize(
      { key: 'accept.replace', comment: ['{0} will be a keybinding, e.g "Enter to replace"'] },
      '{0} to replace'
    ),
  },
  group: 'left',
  ...
  when: ContextKeyExpr.and(
    SuggestContext.HasInsertAndReplaceRange,
    ContextKeyExpr.equals('config.editor.suggest.insertMode', 'replace')
  ),
});
registerEditorCommand(
  new SuggestCommand({
