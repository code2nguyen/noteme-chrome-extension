...
export const editorSuggestWidgetHighlightForeground = registerColor(
  ...
  { dark: listHighlightForeground, light: listHighlightForeground, hc: listHighlightForeground },
  ...
  renderTemplate(container) {
    ...
    data.disposables.add(
      Event.chain(this.editor.onDidChangeConfiguration.bind(this.editor))
        .filter(
          (e) =>
            e.hasChanged(36 /* fontInfo */) ||
            e.hasChanged(97 /* suggestFontSize */) ||
            e.hasChanged(98 /* suggestLineHeight */)
        )
...
ItemRenderer = __decorate(
  [__param(3, IModelService), __param(4, IModeService), __param(5, IThemeService)],
  ItemRenderer
);
  ...
  renderItem(item, explainMode) {
    ...
    if (explainMode) {
      ...
      md += `score: ${item.score[0]}${
        item.word
          ? `, compared '${
              (item.completion.filterText && item.completion.filterText + ' (filterText)') || item.completion.label
            }' with '${item.word}'`
          : ' (no prefix)'
      }\n`;
      md += `distance: ${item.distance}, see localityBonus-setting\n`;
      md += `index: ${item.idx}, based on ${
        (item.completion.sortText && `sortText: "${item.completion.sortText}"`) || 'label'
      }\n`;
...
let SuggestWidget = class SuggestWidget {
  constructor(
    ...
    const kbToggleDetails =
      (_b =
        (_a = keybindingService.lookupKeybinding('toggleSuggestionDetails')) === null || _a === void 0
          ? void 0
    ...
    this.toDispose.add(
      addDisposableListener(this.element, 'click', (e) => {
    ...
    const applyStatusBarStyle = () =>
      this.element.classList.toggle('with-status-bar', this.editor.getOption(96 /* suggest */).statusBar.visible);
    applyStatusBarStyle();
    ...
    const applyIconStyle = () =>
      this.element.classList.toggle('no-icons', !this.editor.getOption(96 /* suggest */).showIcons);
    applyIconStyle();
    ...
    this.list = new List('SuggestWidget', this.listElement, this, [renderer], {
      ...
      accessibilityProvider: {
        ...
        getAriaLabel: (item) => {
          const textLabel =
            typeof item.completion.label === 'string' ? item.completion.label : item.completion.label.name;
    ...
    this.toDispose.add(
      addStandardDisposableListener(this.details.element, 'keydown', (e) => {
  ...
  onListFocus(e) {
    ...
    if (item !== this.focusedItem) {
      ...
      this.currentSuggestionDetails = createCancelablePromise((token) =>
  ...
  adjustDocsPosition() {
    ...
    if (
      this.element.classList.contains('docs-side') &&
