...
var SCSSParser = /** @class */ (function (_super) {
  ...
  SCSSParser.prototype._parseOperator = function () {
    if (
      this.peek(scssScanner.EqualsOperator) ||
      this.peek(scssScanner.NotEqualsOperator) ||
      this.peek(scssScanner.GreaterEqualsOperator) ||
      this.peek(scssScanner.SmallerEqualsOperator) ||
      this.peekDelim('>') ||
      this.peekDelim('<') ||
      this.peekIdent('and') ||
      this.peekIdent('or') ||
  ...
  SCSSParser.prototype._parseRuleSetDeclaration = function () {
    if (this.peek(TokenType.AtKeyword)) {
      return (
        this._parseKeyframe() || // nested @keyframe
    ...
    ); // try css ruleset declaration as last so in the error case, the ast will contain a declaration
  };
  ...
  SCSSParser.prototype._parseSimpleSelectorBody = function () {
    return (
      ...
      _super.prototype._parseSimpleSelectorBody.call(this)
    );
  };
  ...
  SCSSParser.prototype._parseControlStatement = function (parseStatement) {
    ...
    return (
      this._parseIfStatement(parseStatement) ||
